...
const TerminalMode = () => {
  ...
  const [isJDAiInstalled, setIsJDAiInstalled] = useState(() => sessionStorage.getItem('jdai_installed') === 'true');
      ...
      case 'help':
        output = (
          <div className="text-white/70">
            ...
            <span className="text-white font-bold">jdai &lt;query&gt;</span> - Ask the JDAi assistant<br />
        ...
        output = (
          <div className="flex flex-col sm:flex-row gap-6 mt-4 mb-4">
            ...
            <div className="flex flex-col text-[#D7E2EA]/80 gap-1 font-mono">
              <div className="text-white font-bold text-lg mb-2">zameer@portfolio-os</div>
              <div>-------------------</div>
      ...
      default:
        if (cmd.startsWith('cat ')) {
          output = `cat: ${cmd.slice(4)}: Permission denied`;
           ...
           output = 'zameer is not in the sudoers file. This incident will be reported to the cyber police.';
        } else if (cmd === 'date') {
           output = new Date().toString();
        } else if (cmd === 'matrix') {
           output = 'Wake up, Neo... The Matrix has you...';
        } else if (cmd.includes('install jdai')) {
           setIsBusy(true);
           setHistory((prev) => [...prev, { command: input, output: '' }]);
           
           const installSequence = [
              '> Resolving dependencies...',
              '> Fetching packages from jdai-registry...',
              '> Downloading jdai-core-v1.0.0 [====================] 100%',
              '> Extracting neural models...',
              '> Compiling quantum pathways...',
              '> JDAi successfully installed!',
              'Type "jdai" to initialize the AI assistant.'
           ];
           ...
           sessionStorage.setItem('jdai_installed', 'true');
           setIsJDAiInstalled(true);
           setIsBusy(false);
           return;
        } else if (cmd === 'jdai' || cmd.startsWith('jdai ')) {
           if (!isJDAiInstalled) {
             output = (
               <span className="text-white/60">
                 Error: JDAi is not installed. Please run <span className="text-white font-bold">"zameer install jdai"</span> first.
               </span>
             );
           } else if (cmd === 'jdai') {
              output = '> GREETING PROTOCOL: ENGAGED\n> Hello. I am JDAi. Usage: jdai <query>\n> Example: jdai projects, jdai skills, jdai contact';
           } else {
